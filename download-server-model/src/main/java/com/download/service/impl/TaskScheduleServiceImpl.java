...
public class TaskScheduleServiceImpl implements TaskScheduleService {
    ...
    public void taskPolling(SendTransferMsgDTO sendTransferMsgDTO) {
        ...
        //String url = sendTransferMsgDTO.getUrl();
        //LambdaQueryWrapper<Transfer> wrapper = new LambdaQueryWrapper<Transfer>().eq(Transfer::getUrl, url);
        ...
        if (res != null){
            ...
            }else if (taskStatus.equalsIgnoreCase("Downloaded") && (taskThread == null || !taskThread.isAlive())){
                //创建任务
                taskThread.start();
                taskThreads.put(taskId,taskThread);
            } else if (taskStatus.equalsIgnoreCase("pending") && (taskThread != null && taskThread.isAlive())) {
                // 关闭任务线程实现任务的关闭
                taskThread.stop();
